...
import { KitchenProducts } from "../assets/asset";
import NavLinks from '../Components/NavLinks';
import Filter from '../Components/Filter';
import KitchenImages from '../assets/Images/Kitchen.webp';
...
function Kitchen() {
  ...
  // Prefix Kitchen product IDs
  const kitchenData = KitchenProducts.map((item, index) => ({
    ...item,
    id: item.id?.toString().startsWith("kitchen-") ? item.id : `kitchen-${index + 1}`
  }));

  return (
    <div className="w-full flex flex-col justify-center items-center overflow-x-hidden">
      <div className="w-full hidden md:flex h-40 relative bg-[#757575] ">
        <h1 className="absolute top-12 left-[40%] text-white font-bold text-xl">Kitchen</h1>
        <p className="absolute top-20 left-[35%] text-white text-sm">
      ...
      <div className="w-4/12 sm:w-5/12 lg:w-2/12 flex justify-center items-center flex-col gap-y-2 mb-4">
        <img className="w-6/12 h-6/12" src={KitchenImages} alt="Kitchen Gadgets" />
        <h1 className="w-8/12 ml-2 text-center">Kitchen Gadgets</h1>
      </div>
            ...
            >
              <div className="w-full h-3/12 lg:w-52 lg:h-44 flex justify-center items-center bg-[#E5E5E5] hover:opacity-85">
                <img
                  src={item.img}
                  alt=""
                  className="w-full object-cover shadow-lg rounded-md transition-transform group-hover:scale-105 cursor-pointer duration-700 delay-200 ease-in-out"
                />
                <div className="absolute top-28 left-1/2 -translate-x-1/2 transition-all duration-300 ease-in-out">
                  {hoveredId === item.id && showCartId !== item.id && (
                    <Link
                      ...
                      className="bg-white py-1 px-4 rounded-3xl text-black shadow-lg transition-transform duration-500 hover:scale-110"
                  ...
                  {showCartId === item.id && (
                    <FaShoppingCart
                      className="bg-[#861F3D] w-24 py-1 text-2xl px-4 rounded-3xl text-white shadow-lg transition duration-500"
